  ...
  describe Parser, 'parse flow' do
    ...
    describe :paragraphs do
      ...
      describe 'one-liner' do
        let(:source){"some text"}
        it{should == [Paragraph.new(Text.new('some text'))]}
      end

      describe 'continuous paragraph' do
        let(:source){"some text\nor some other text"}
        it{should == [Paragraph.new(Text.new("some text or some other text"))]}
      end

      describe 'several paragraphs' do
        let(:source){"some text\n\nor some other text"}
        it{should == [
          Paragraph.new(Text.new("some text")),
          Paragraph.new(Text.new("or some other text"))
        ]}
      end
      ...
      describe 'with end regexp' do
        ...
        it{should == [
          Paragraph.new(Text.new("some text")),
          Paragraph.new(Text.new("or some"))
        ]}
      end
    ...
    describe 'long inline' do
      ...
      describe 'inline, then paragraphs' do
        let(:source){"some text\nor some other text"}
        it{should == [Text.new("some text"), Paragraph.new(Text.new("or some other text"))]}
      end

      describe 'inline, then paragraphs - with end regexp' do
        let(:source){"some text\nor some}} other text"}
        subject{parser.long_inline(/}}/)}
        it{should == [Text.new("some text"), Paragraph.new(Text.new("or some"))]}
      end
    end
  end
end
